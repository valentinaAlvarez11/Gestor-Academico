...
export class ProfesorService {
  ...
  async create(dto: ProfesorDto): Promise<any> {
    if (!dto.departamento || dto.departamento.id === undefined) {
      throw new BadRequestException({ message: 'ID de departamento no puede ser undefined' });
    }
    const departamento = await this.departamentoService.findById(dto.departamento.id);
    if (!departamento) {
